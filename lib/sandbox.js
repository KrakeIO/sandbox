...
// @param : socket_host:String    - the host of the Socket server
// @param : socket_options:Object - the settings for setting up the Socket Server
// @param : interface:Object      - the interfaces for which to call the interactions from
...
SandBox.prototype.getJsonQueryObject = function () {
  var self = this;  
  data = self.interface.queryHolder().val();
  try {
  ...
  if(data_obj) {
    self.interface.queryHolder().val(sb.formatJSON(data_obj));
    return data_obj;    
  } else {
    return false;    
  }
...
SandBox.prototype.state3 = function() {
  ...
  self.interface.queryHolder().bind('input cut paste', function() {
    self.state4();
  });
};

// @Description: when transiting to state 1 again
SandBox.prototype.state4 = function() {
  var self = this;    
  self.state = 4;  
  self.interface.queryHolder().unbind
  self.state1();  
};
...
SandBox.prototype.addDataRow = function(data_string) {
  ...
  try {
    ...
    self.current_schema.forEach(function(schema) {
      if(self.url_cols.indexOf(schema) > -1) {
        dataVal = jQuery("<a>").attr({ href : data_obj[schema], target : "_blank" }).html( data_obj[schema] )
        columns.push({ data: dataVal });        
      } else {
...
SandBox.prototype.setSchema = function(query_json_obj) {
  var self = this;
  self.url_cols = [];
  self.current_schema = self.getSchemaRecursive(query_json_obj);
  self.current_schema.push('origin_url');
  self.current_schema.push('origin_pattern');
  self.url_cols.push('origin_url');
  self.url_cols.push('origin_pattern');
...
SandBox.prototype.filterSchema = function(schema, object) {
  if(object.column_filter) { 
    schema = schema.filter(function(e){
...
SandBox.prototype.getSchemaRecursive = function(query_object) {
  var self = this;  
  var columns = [];
  if(query_object.columns)
    columns = columns.concat(self.getColumnsRecursive(query_object.columns));

  if(query_object.permuted_columns && query_object.permuted_columns.handles)
    columns = columns.concat(self.getColumnsRecursive(query_object.permuted_columns.handles));

  if(query_object.permuted_columns && query_object.permuted_columns.responses)
    columns = columns.concat(self.getColumnsRecursive(query_object.permuted_columns.responses));

  if(query_object.post_data)
    columns = columns.concat(this.getPostDataColumns(query_object.post_data));

  if(query_object && query_object.data) {
    Object.keys(query_object.data).forEach(function(col_name) {
      if(columns.indexOf(col_name) < 0 ) {
...
SandBox.prototype.getPostDataColumns = function(post_data) {
  var columns;
  columns = [];
  if (Array.isArray(post_data)) {
    post_data.forEach(function(curr_post_data) {
...
SandBox.prototype.getUniqueColumns = function(raw_columns) {
  var unique_cols;
  unique_cols = [];
  raw_columns = raw_columns || [];
  raw_columns.forEach(function(col_name) {
    if(unique_cols.indexOf(col_name) < 0) {
...
*/
SandBox.prototype.getColumnsRecursive = function(column_array) {
  var self = this;
  var schema_array = [];
  var url_values = [];
  ...
  if(column_array) {
    for(var x = 0; x < column_array.length ; x++ ) {
      ...
      if(curr_column.options) { // Has nested column
        ...
        schema_array = schema_array.concat(self.getSchemaRecursive(curr_column.options));
    ...
    self.url_cols = self.url_cols.concat(url_values);
  }
  return schema_array;
};
