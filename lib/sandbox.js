...
var SandBox = function(socket_host, socket_options) {
  var self = this;
...
SandBox.prototype.setSocketEventListeners = function() {
  ...
  self.socket.on('json results returned', function(data){
    ...
    if(self.prepare_to_switch_pane) {
      ...
      SandBox.interface.showPane('datatable');
    }
  });
...
SandBox.prototype.setDocumentEventListeners = function() {
  ...
  SandBox.interface.taskButton().click(function() {
    if(!self.socket) {
      self.setupSocket();
    }
    
    // Starts the scraping operation
    if(SandBox.interface.taskButton.readsStart()) {
      self.prepare_to_switch_pane = true;
      SandBox.interface.showPane('logs'); 
      self.clearDisplay();
      ...
      self.verifyDefinition(query_json_obj, function(is_valid, error_msg) {
        if(is_valid) {
          ...
          SandBox.interface.displayError(error_msg);
          SandBox.interface.showPane('edit');
          self.state1();
        }
      });
...
SandBox.prototype.getJsonQueryObject = function () {
  data = SandBox.interface.queryString();
  try {
  ...
  if(data_obj) {
    SandBox.interface.setQueryString(sb.formatJSON(data_obj));
    return data_obj;    
  } else {
    return false;    
  }
...
SandBox.prototype.addToLog = function(data) {
  SandBox.interface.incrementCounter('logs');
  SandBox.interface.appendLog(data);
};
...
SandBox.prototype.state3 = function() {
  ...
  SandBox.interface.queryString().bind('input cut paste', function() {
    SandBox.interface.queryString().unbind
    self.state1();
  })
};
...
SandBox.prototype.resetDataTable = function() { 
  var self = this
  SandBox.interface.clearData();
  SandBox.interface.setHeader(self.current_schema);
};
...
SandBox.prototype.addDataRow = function(data_string) {
  SandBox.interface.incrementCounter('data-row');
  var self = this;
  var columns = [];
  try {
    ...
    SandBox.interface.appendRecord(columns);
  } 
  catch(exception) {
...
SandBox.prototype.setSchema = function(query_json_obj) {
  ...
  return SandBox.filterSchema(self.current_schema, query_json_obj);
};
